...
import { IERC4626, IERC20Metadata } from "src/interfaces/IERC4626.sol";
...
contract BaseRewardPool4626 is BaseRewardPool, ReentrancyGuard, IERC4626 {
    ...
    function withdraw(
        ...
        if (msg.sender != owner) {
            _approve(owner, msg.sender, _allowances[owner][msg.sender] - (assets));
        }
     ...
     * of underlying tokens to `receiver`.
     */
    function redeem(uint256 shares, address receiver, address owner) external virtual override returns (uint256) {
    ...
    function name() external view override returns (string memory) {
        return string(abi.encodePacked(IERC20Metadata(address(stakingToken)).name(), " Vault"));
    }
    ...
    function symbol() external view override returns (string memory) {
        return string(abi.encodePacked(IERC20Metadata(address(stakingToken)).symbol(), "-vault"));
    }
